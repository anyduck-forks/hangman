    ...
    (wordLength = null, currentGuesses = null, currentIsWon = null) => {
      ...
      history.forEach((game) => {
        ...
        if (game.missedGuesses !== undefined) {
          ...
          count = game.gameState.guesses.filter(
            (l) => !game.gameState.word.includes(l),
          ).length;
        } else {
      ...
      const maxKey = Math.max(
        6,
        ...numericKeys,
        currentIsWon && typeof currentGuesses === "number" ? currentGuesses : 0,
      );
      const bars = [];
      ...
      for (let i = 0; i <= maxKey; i++) {
        ...
        const percentage =
          maxFreq > 0 ? Math.round((count / maxFreq) * 100) : 0;
